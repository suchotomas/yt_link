...
# #
# # path= '/mnt/data/palpatine/DATASETS/YT_LINK/HARD_COPY/youtube.json'
# # path_list = load_json(path)
# # exts = {}
...
# # export_list = []
# # for record in data:
# #     if record['dst'].split('/', 1)[0] == 'export':
# #         export_list.append(record)
# #     else:
# #         youtube_list.append(record)
# #
# #
# # save_json('/mnt/data/palpatine/SAMPLES/YT_LINK/REF/workdir/export_to_process.json', export_list)
# # save_json('/mnt/data/palpatine/SAMPLES/YT_LINK/REF/workdir/youtube_to_process.json', youtube_list)
...
# # CreateImages().process(yt_list, ex_list, workdir)
...
matches_path = '/mnt/data/palpatine/DATASETS/YT_LINK/workdir/matches.pickle'
path_out = '/mnt/data/palpatine/DATASETS/YT_LINK/workdir/matches_mse.pickle'
workdir = '/mnt/data/palpatine/DATASETS/YT_LINK/workdir'

# m = MatchByDuration(export_path=export_path, youtube_path=youtube_path, workdir=workdir)
# matches_path = m.get_duration_lists()


matches = t.load_pickle(matches_path)
diffs = []

done = 0
counter = 0
for  ida, match in tqdm(matches.items(), ascii=True, desc='process {} lines'.format(len(matches))):
    counter +=1
# for ida, match in matches.items():
    matches[ida].mse = []
    a_path =os.path.join(workdir,  match.ida, 'image.png')
    a_info_path =os.path.join(workdir,  match.ida, 'info.json')
    ...
    if len(match.diff):
        match.diff = [d for d in match.diff if d < 100]

        if len(match.diff):
            for idb in match.idb:
                b_path =os.path.join(workdir,  idb, 'image.png')
                b_info_path = os.path.join(workdir, idb, 'info.json')
    ...
    # print('saved')
print( 'done {}/{}'.format(done, len(matches)))
        
# lens.append(len(match.diff))
# diffs = sorted(diffs, reverse=True)
#
# print(diffs[:50])
# print('median', np.median(np.array(diffs)))
# print(len(diffs))
# print()